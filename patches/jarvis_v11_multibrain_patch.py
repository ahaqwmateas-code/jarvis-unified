...
SKILL = {
    "name": "brain",
    "description": "brain / brain key <any provider key> / brain local",
...
def _default_model(p):
    return {"groq": "llama-3.3-70b-versatile", "openai": "gpt-4o-mini",
            ...
            "openrouter": "meta-llama/llama-3.1-8b-instruct:free"}.get(p, "")
...
def run(args, ctx):
    ...
    if low in ("cloud", "groq", "openai", "anthropic", "google", "openrouter"):
...
# 2) insert ask_cloud (multi-provider) if not present
if "def ask_cloud(" not in src:
    ...
    changed = True
    print("core.py: multi-provider ask_cloud added")
else:
    print("core.py: ask_cloud already present")
...
print("brain skill upgraded (multi-provider)")
