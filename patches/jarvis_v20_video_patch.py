...
VIDEO_SKILL = r'''import os, json, time, base64, subprocess, shutil, urllib.request, urllib.parse
...
# ensure gallery static serving exists (in case a previous patch didn't add it)
...
# overwrite the video skill with the real generator
os.makedirs(os.path.join(J, "skills"), exist_ok=True)
open(os.path.join(J, "skills", "video.py"), "w").write(VIDEO_SKILL)
print("video skill upgraded (real MP4 generation: free frames + ffmpeg Ken Burns)")
